...
SESyncResult SESync(const std::vector<RelativePoseMeasurement> &measurements,
  ...
  switch( algType )
  {
    case AlgType::StiefelSync:
      {
        results = SESync::StiefelSync(measurements, options, Y0);
        break;
      }
    case AlgType::CartanSync:
      {
        results = SESync::CartanSync(measurements, options, Y0);
        break;
      }
  }
  return results;
...
bool gradient_norm_stopping_criterion_StiefelSync(
  ...
  const StiefelSyncProblem *SESync_problem_ptr =
      static_cast<const StiefelSyncProblem *>(problem);
...
SESyncResult StiefelSync(const std::vector<RelativePoseMeasurement> &measurements,
                         const SESyncOpts &options, const Eigen::MatrixXd &Y0) {
  ...
  StiefelSyncProblem problem(LGrho, A, T, Omega, options.use_Cholesky);
  ...
  for (unsigned int r = options.r0; r <= options.rmax; r++) {
    ...
    RTR.StopPtr = &gradient_norm_stopping_criterion_StiefelSync;
