 ...
 * values */
  SESyncOpts(double gradient_norm_tolerance = 1e-2,
             ...
             bool Cholesky = true, bool precon = true, bool verbose_output = false)
      : tolgradnorm(gradient_norm_tolerance),
        ...
        use_Cholesky(Cholesky), use_preconditioning(precon),
 ...
 * Internally choose either the version by Rosen et al. (here StiefelSync)
 * or by Briales and Gonzalez (here CartanSync)
 */
...
/** Given a vector of relative pose measurements specifying a special Euclidean
 * synchronization problem, performs synchronization using the StiefelSync algorithm
 */
SESyncResult StiefelSync(
                    const std::vector<RelativePoseMeasurement> &measurements,
                    const SESyncOpts &options = SESyncOpts(),
                    const Matrix &Y0 = Matrix());

/** Given a vector of relative pose measurements specifying a special Euclidean
 * synchronization problem, performs synchronization using the Cartan-Sync algorithm
 */
SESyncResult CartanSync(
                    const std::vector<RelativePoseMeasurement> &measurements,
                    const SESyncOpts &options = SESyncOpts(),
                    const Matrix &Y0 = Matrix());
} // namespace SESync
