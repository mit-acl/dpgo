...
RGDMaster::RGDMaster(QuadraticProblem* p, Matrix Y0)
    : problem(p), Y(Y0), stepsize(0.001), updateRate(10000) {
  d = problem->dimension();
...
void RGDMaster::solve(unsigned num_threads) {
  ...
  if (numPosesPerWorker == 0) {
    cout << "Idle workers detected. Try decrease the number of workers."
         << endl;
    return;
  }
  ...
  cout << "Master finished. Total number of writes: " << numWrites
       << ". Elapsed time: " << elapsedTimes.back() / (float)1000 << " seconds."
       << endl;
}
